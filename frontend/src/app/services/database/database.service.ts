...
export class DatabaseService {
  ...
  async clearScores(scores: User[]) {
    ...
    const users = await this.selectUsers().catch(error => {
      throw error;
    });
    ...
    // loop through array and create email list
    let emailTo = '';
    for (const user of users) {
      if (emailTo === '') {
        emailTo = user.email;
    ...
    // when finished send email with high scores
    axios
      .post(environment.sendEmail, {
        to: emailTo,
        scores: {
          firstPlace: `${saveScores.firstPlace.firstName} ${saveScores.firstPlace.lastName}`,
          secondPlace: `${saveScores.secondPlace.firstName} ${saveScores.secondPlace.lastName}`,
          thirdPlace: `${saveScores.thirdPlace.firstName} ${saveScores.thirdPlace.lastName}`
        }
      })
      .catch(error => {
        throw error;
